...
function moveDiceToCamera() {
    ...
    diceArray.forEach((dice, index) => {
        ...
        // Y축 오일러 각만 0도로 설정
        const currentRotation = dice.mesh.rotation; // 현재 회전 값을 가져옴
        dice.mesh.rotation.set(0, 0, -Math.PI/2); // Y축만 0도로 설정
        dice.body.quaternion.copy(dice.mesh.quaternion); // 물리 바디와 동기화
