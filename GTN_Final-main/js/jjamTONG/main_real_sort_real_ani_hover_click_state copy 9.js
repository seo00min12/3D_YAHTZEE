...
// clickstate기능을 넣어 0일때 와 1일때의 상황을 전개 해야함
// 처음 주사위를 굴릴때는 clickstate상태가 0인채로 시작 hover기능과 겹쳐 홀수번누르면 1 ,짝수번 누르면 0 으로 구현 clickstate가 1인 애들은 제외하고 주사위를 굴림
// 이때 clickstate가 1인애들은 점수판에 표기가 고정되며 0인 애들은 다시 굴림




//copy3 -> copy4 
// throw 버튼을 눌러야만 주사위가 굴러가게 수정 즉 html창을 열때 주사위가 굴러가는 것을 방지

// copy5 -> copy6
// throwDice()가 호출될때는 throw!버튼을 반투명상태로 만들어서 비활성화 하기기
// 그리고  addMouseInteractionToDice() 함수가 끝나면 그때 다시 불투명 상태로 만들어서 그때 다시 throw!버튼을 누를수있게 만들기


// copy6 -> copy7
// 주사위에 click이벤트가 적용되었을때 특정위치로 애니메이션 효과를 넣어서 충분이 click되었음을 사용자에게 시각적으로 보여주는 애니메이션 추가



// 7은 파기 예정이 될수도있음 7은 버그 덩어리



// copy7 -> copy8 
// clickstate가 여백을 클릭하면 1에서 0이 되는 버그 수정

//1. clickListener가 중복 실행됨 → 여러 번 상태 변경
    // addMouseInteractionToDice()가 여러 번 실행되면서 clickListener가 중복 등록되었음.
    // 클릭할 때마다 여러 개의 이벤트가 실행되어 예상치 못한 clickState 변경이 발생했음
    //removeEventListener를 사용하여 중복 리스너를 방지함

//2. clickState를 여러 곳에서 변경함 → 이전 주사위의 상태가 초기화됨
    // clickState를 throwDice() 등 여러 곳에서 변경하면서 새로운 주사위를 클릭하면 기존 주사위의 clickState가 초기화되는 문제 발생.
    // ✅ 해결: clickState를 clickListener에서만 변경하도록 수정하여 상태를 개별적으로 유지

//3. mousemove(hover) 이벤트가 주사위 위치를 변경함
    // clickState === 1인 주사위도 mousemove 이벤트의 영향을 받아 y 값이 변하는 문제 발생.
    // ✅ 해결: mousemove 이벤트에서 clickState === 1인 경우 hover 이벤트를 무시하도록 조건 추가.

//4. 여백을 클릭해도 clickListener가 실행됨
    // 기존 코드에서는 화면의 빈 공간(여백)을 클릭해도 clickListener가 실행되었음.
    // window.addEventListener('click', dice.mesh.__clickListener); 때문에 모든 클릭이 clickListener로 전달됨.
    // ✅ 해결: Raycaster를 이용하여 마우스가 실제 주사위를 클릭한 경우에만 clickState가 변경되도록 수정.
...
// 버그를 수정함 문제가 뭐였냐면 hover이벤트의 문제가 있었음 dice.mesh.__hoverListener가 마우스를 움직일때마다 
// 실행이되니까 문제임 if (dice.body.userData.clickState === 1) return;가 있어서 clickstate가 1인 주사위 두번째 throw!버튼을 
// 누를때부터 hover이벤트가 적용되는거였다 그렇다고 if (dice.body.userData.clickState === 1) return;를 지우면 주사위를 클릭하고 
// 마우스를 움직일때 
// else if (isHovered) {
//     isHovered = false;
//     animateDiceOnHover(dice, false); // 
// }
// 이코드때문에 원래 위치로 이동하는거인것이다
// else if (isHovered) {
//     isHovered = false;
//     animateDiceOnHover(dice, false); // 원래 위치로 복원
// }
// 주사위 보관 위치에있는 주사위는 해당 이벤트를 적용시키지 않게 함

// => 최적화(고려사항) : 혹시 그냥 마우스 위치를 분석하지말고 hover이벤트를 지우고 click이벤트만 설정하는건 어떤지 싶자 raycast인가 뭔가를 이용하면 될지도 모른다



// 해야할거 hover이벤트는 movetocamerawithanimation이벤트가 끝나고 1초뒤에 실행하는걸로 하는게 좋겠다 바로 마우스를 가져다가 대면 버그가 생긴다



//copy9 -> copy10  희망사항
// 점수를 먼저 계산 → 점수판과 주사위 순서 동기화 → 애니메이션 실행
// moveDiceToCameraWithAnimation에서 clickstate가 1인 주사위는 회전정렬은 하되 애니메이션은 하지않는 코드만들기
// 회전 정렬이 끝나고 애니메이션을 실행하기 전에 score보드와 주사위 배열순서의 동기화가 한번 됐으면 좋겠음
...
// 02-07 (throw버튼을 눌러야만 주사위가 굴러가게 변경)
...
function createDice() {
    const mesh = diceMesh.clone();                    // 주사위 매쉬를 복사
    ...
    body.velocity.setZero();       // 초기 속도 0
    body.angularVelocity.setZero(); // 초기 회전 속도 0
    body.allowSleep = true;         // 움직이지 않으면 자동으로 멈춤 (중요!)
    body.sleep();                   // 초기 상태에서 즉시 멈춤 (이 코드 추가!)
    

    body.userData = { clickState: 0 };                // 주사위의 상태 초기화 (clickState: 0)
    physicsWorld.addBody(body);                       // 물리 세계에 바디 추가

    return { mesh, body };                            // 생성된 주사위 객체 반환
}
...
function throwDice() {
    ...
    let animationsCompleted = 0; // 🟢 애니메이션 완료 상태 초기화

    // 🛑 Throw 버튼 비활성화 및 반투명 처리
    rollBtn.disabled = true; // 버튼 비활성화
    rollBtn.style.opacity = "0.5"; // 반투명 처리
    rollBtn.style.cursor = "not-allowed"; // 커서 변경
    ...
    const prevClickCount = prevScores.filter((_, idx) => diceArray[idx]?.body.userData.clickState === 1).length;
    ...
    if (!firstThrow) {
        newScores = newScores.filter((score, idx) => diceArray[idx].body.userData.clickState === 1);
    }
    ...
    // moveDiceToCameraWithAnimation(); // 애니메이션 함수 실행

    // moveDiceToCameraWithAnimation() 실행 후 clickstate 초기화 및 복구
    setTimeout(() => {
        diceArray.forEach(dice => {
        ...
        diceArray.forEach((dice, index) => {
            if (index < prevClickCount) {
                dice.body.userData.clickState = 1;
            }
        });
    }, 500); // moveDiceToCameraWithAnimation() 실행 후 적용
}
...
function moveDiceToCameraWithAnimation() {
    ...
    diceArray.forEach((dice, index) => {
        ...
        // 🟢 주사위의 초기 위치를 저장 (copy7)
        dice.body.userData.initialPosition = targetPosition;
...
function addMouseInteractionToDice() {
    ...
    // 🟢 Throw! 버튼 불투명 상태로 활성화
    rollBtn.disabled = false; // 버튼 활성화
    rollBtn.style.opacity = "1"; // 불투명 상태로 복원
    rollBtn.style.cursor = "pointer"; // 커서 복구
    ...
    diceArray.forEach((dice) => {
        ...
        dice.mesh.__hoverListener = (event) => {
            if (!isInteractionEnabled) return; // 주사위를 굴리는 중이면 hover 애니메이션 실행 안 함
            ...
            if (intersects.length > 0) {
                if (!isHovered) {
                    isHovered = true;
                    animateDiceOnHover(dice, true); // hover 애니메이션 실행
                }
            } 
            // 🛑 주사위가 보관 위치(`clickState === 1`)에 있을 경우 원래 위치로 복귀하지 않도록 수정
            else if (isHovered && dice.body.userData.clickState === 0) { 
                isHovered = false;
                animateDiceOnHover(dice, false); // 원래 위치로 복원
            }
        };
        ...
        dice.mesh.__clickListener = (event) => {
            // 🛑 Raycaster를 이용해 클릭한 대상이 실제 주사위인지 확인
            mouse.x = (event.clientX / window.innerWidth) * 2 - 1;
            mouse.y = -(event.clientY / window.innerHeight) * 2 + 1;
            ...
            if (intersects.length === 0) return; // 🛑 주사위를 클릭하지 않은 경우 함수 종료
        
            if (!isHovered) return; // hover 상태가 아니면 클릭 이벤트를 무시
        
            const index = diceArray.indexOf(dice);
            const result = dice.body.userData.result || "?";
            ...
            // 🔹 Three.js 메쉬 위치 강제 동기화
            dice.mesh.position.copy(dice.body.position);
        
            updateButtonState(); // 버튼 상태 업데이트
        
            console.log(`${index + 1}번째 주사위 클릭됨 (clickState: ${dice.body.userData.clickState})`);
        };
        ...
        window.addEventListener('click', dice.mesh.__clickListener);
                ...
                // mouseleave 이벤트 리스너
                dice.mesh.__leaveListener = () => {
                    if (!isInteractionEnabled) return; // 인터랙션이 비활성화된 경우 종료
                    isHovered = false; // hover 상태 해제
                    animateDiceOnHover(dice, false);
                };
                window.addEventListener('mouseleave', dice.mesh.__leaveListener);
    });
}
...
// 02-07 clickstate가 1일때 throw!버튼 -> save!버튼으로 전환
// save! 버튼을 누르면 savedScores에 리스트배열을 담고 다시 throwDice실행!


const savedScores = []; // 점수를 저장할 리스트 객체 생성
...
// 처음 주사위를 굴렸을때는 지금 상태 그대로 유지 다시 throw를 했을때 모든 값의 clickstate를 0으로 지정하고 주사위를 hold한 수 만큼 앞에서 부터 clickstate를 1로 변경 
// ex) 1,2,3,4,5 가 나오고 3과4를 click한뒤 다시 throw를 누르면 모든 주사위의 clickstate가 0으로 초기화 그리고 이전의 clickstate 수만큼 앞에서 
// clickstate를 1로 변경
