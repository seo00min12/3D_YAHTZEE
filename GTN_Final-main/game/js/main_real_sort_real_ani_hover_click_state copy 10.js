...
// 주사위'인덱스'와 score의 '인덱스'를 동기화 시키는 코드를 작성해야함

//  기존 diceArray를 유지하면서 clickState === 1인 주사위가 앞으로 정렬됨
//  다시 throw! 버튼을 눌러도 고정된 주사위가 항상 앞쪽 유지
//  성능을 고려한 최적화된 방식

// 현재 clickState === 1인 주사위(예: 3번째, 4번째)가 다시 throw! 버튼을 누르면 새로운 배열에서 첫 번째, 두 번째 주사위로 이동해야 한다.

// 즉, 다음 번 throw! 실행 시:

// 이전 clickState === 1이었던 주사위들이 먼저 배치된다.
// 나머지 주사위들은 그 뒤에 순서대로 정렬된다.

// 0214
// 보관위치와 주사위 인덱스 번호 연동 완료 이제 주사위의 인덱스번호가 바뀌면 해당 인덱스 번호에 맞는 보관위치에 즉각적으로 이동



// 두번째 주사위 굴리는 상황부터 보관위치에 있는 주사위가 hover가 적용되는 버그 발생
// hoverListener를 추가할 때 항상 clickState === 1인지 먼저 체크해서 hover를 무시
// copy10을 마지막으로 기본틀 완성
...
// 3,5 clickstate를 1로
// throw!를 누르면 3 5는 잘 유지 하지만 score에서 1,2번째를 clickstate를 score에 인식
// 내 copy10코드를 보면 throw를 누르면 모든 clickstate 를 0으로 바꾸고 이전에 clickstate가 1인 숫자만큼 주사위배열의 앞에서 부터 clickstate값을 다시 1로 바꾸라고 했다
// 하지만 score가 너무 빨리 이를 실행해버리는거 같다
...
// function syncDiceWithScores() {
//     const scores = scoreResult.dataset.scores ? scoreResult.dataset.scores.split(',').map(Number) : [];

//     // 주사위의 결과(result) 기준으로 diceArray 정렬
//     diceArray.sort((a, b) => {
//         const scoreA = a.body.userData.result || 0;
//         const scoreB = b.body.userData.result || 0;
//         return scores.indexOf(scoreA) - scores.indexOf(scoreB);
//     });

//     console.log("✅ 주사위 배열과 점수 배열 동기화 완료!");
// }
