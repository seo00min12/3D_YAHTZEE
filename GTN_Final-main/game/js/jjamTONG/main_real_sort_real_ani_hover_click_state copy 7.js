...
function addMouseInteractionToDice() {
    ...
    diceArray.forEach((dice) => {
        ...
        dice.mesh.__hoverListener = (event) => {
            console.log("🖱️ 마우스 이동 감지됨!");
        
            if (!isInteractionEnabled) return; // 주사위를 굴리는 중이면 hover 애니메이션 실행 안 함
            if (dice.body.userData.clickState === 1) return; // 🛑 보관된 주사위는 hover 이벤트 차단
        ...
        dice.mesh.__clickListener = () => {
            ...
            if (dice.body.userData.clickState === 1) {
                // 🟢 보관 위치로 이동
                const targetPosition = storagePositions[index];
                ...
                console.log(`주사위 ${index + 1}가 보관 위치로 이동했습니다.`);
            } else {
                // 🟢 개별 주사위를 초기 위치로 복구
                const initialPosition = dice.body.userData.initialPosition;
                dice.body.position.set(initialPosition.x, initialPosition.y, initialPosition.z);
                dice.body.velocity.set(0, 0, 0);
                dice.body.angularVelocity.set(0, 0, 0);
                ...
                console.log(`주사위 ${index + 1}가 초기 위치로 복구되었습니다.`);
            }
        };
