...
function throwDice() {
    ...
    // clickState가 0인 주사위만 굴리기
    diceArray.forEach((d, dIdx) => {
        if (d.body.userData.clickState === 1) return; // clickState가 1인 주사위는 굴리지 않음

        d.body.velocity.setZero();                      // 기존 속도를 제거
        d.body.angularVelocity.setZero();               // 기존 회전 속도를 제거

        d.body.position = new CANNON.Vec3(6, dIdx * 1.5, 0); // 초기 위치 설정
        d.mesh.position.copy(d.body.position);          // 메쉬와 물리적 바디 위치 동기화

        d.mesh.rotation.set(2 * Math.PI * Math.random(), 0, 2 * Math.PI * Math.random()); // 무작위 회전
        d.body.quaternion.copy(d.mesh.quaternion);      // 메쉬와 물리적 바디 회전 동기화

        const force = 3 + 5 * Math.random();            // 3에서 8 사이의 랜덤 힘 생성
        d.body.applyImpulse(
            new CANNON.Vec3(-force, force, 0),          // 힘을 적용 (왼쪽으로 밀고 위쪽으로 튕김)
            new CANNON.Vec3(0, 0, .2)                   // 힘이 적용되는 위치를 설정
        );

        d.body.allowSleep = true;                       // 주사위가 정지하면 시뮬레이션 제외
    });
}
