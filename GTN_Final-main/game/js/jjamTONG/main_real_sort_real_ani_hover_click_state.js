...
function throwDice() {
    ...
    diceArray.forEach((d, dIdx) => {
        ...
        d.body.allowSleep = true; // 주사위가 멈추면 시뮬레이션 제외
    });

    // 점수 UI 업데이트
    scoreResult.dataset.scores = newScores.map(score => score || "").join(',');
    scoreResult.textContent = newScores.filter(score => score !== null).join(',');
