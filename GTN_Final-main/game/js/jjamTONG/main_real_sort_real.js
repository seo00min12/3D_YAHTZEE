...
function moveDiceToCamera() {
    const spacing = 2; // 주사위 간격
    diceArray.forEach((dice, index) => {
        const xPosition = (index - Math.floor(diceArray.length / 2)) * spacing;

        // 주사위 위치 설정
        dice.body.position.set(xPosition, 3.8, 0);
        dice.body.velocity.setZero();
        dice.body.angularVelocity.setZero();

        // showRollResults() 값으로 주사위 윗면 값 가져오기
        const result = parseInt(dice.body.userData.result); // 주사위 결과 값 (1~6)
        console.log(`Dice ${index + 1} result: ${result}`); // 디버깅용

        // 결과에 따른 회전 값 설정
        switch (result) {
            case 1:
                dice.mesh.rotation.set(0, 0, 0); // 카메라 시야에서 정면으로 1이나옴
                break;
            case 2:
                dice.mesh.rotation.set(0, 0, Math.PI / 2); // 카메라 시야에서 정면으로 2가나옴
                break;
            case 3:
                dice.mesh.rotation.set(-Math.PI / 2, 0, 0); // 카메라 시야에서 정면으로 3이나옴
                break;
            case 4:
                dice.mesh.rotation.set(Math.PI / 2, 0, 0); // 카메라 시야에서 정면으로 4가나옴
                break;
            case 5:
                dice.mesh.rotation.set(0, 0, -Math.PI / 2); // 카메라 시야에서 정면으로 5가나옴
                break;
            case 6:
                dice.mesh.rotation.set(Math.PI, 0, 0); // 카메라 시야에서 정면으로 6이나옴
                break;
        }
