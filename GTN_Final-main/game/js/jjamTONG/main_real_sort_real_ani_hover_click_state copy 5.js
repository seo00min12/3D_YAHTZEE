...
function throwDice() {
    ...
    isInteractionEnabled = false; // 🛑 주사위를 굴리는 동안 hover 차단(두번째 throw!버튼을 누를때 hover이벤트가 주사위를 굴리는 도중에 발생하는 버그를 해결하기위해
                                    // 해당 코드가 필요 🛑 표시한게 그 버그를 해결해 주는 코드)

    // 🛑 기존 hover 이벤트 제거(이걸 적용하지않으면 두번째 throw!버튼을 누를때 mousemove,click,mouseleave,hover가 적용되는 버그가 발생 그래서 해당 코드가 필요!(1/31수정))
    ...
    diceArray.forEach((d, dIdx) => {
        ...
        d.body.wakeUp();  // 🛑 여기 추가 → 주사위를 깨워야 움직일 수 있음!
    ...
    moveDiceToCameraWithAnimation();

    // moveDiceToCameraWithAnimation() 실행 후 clickstate 초기화 및 복구
    setTimeout(() => {
        // 모든 주사위 clickstate를 0으로 초기화
        diceArray.forEach(dice => {
            dice.body.userData.clickState = 0;
        });

        // 이전에 클릭한 주사위 개수만큼 score의 앞부분을 clickstate = 1로 복구
        diceArray.forEach((dice, index) => {
...
dice.mesh.__clickListener = () => {
    ...
    // 주사위의 클릭 상태 업데이트
    const result = dice.body.userData.result || "?"; // result 값이 없으면 "?"로 대체
    dice.body.userData.clickState = dice.body.userData.clickState === 1 ? 0 : 1;
    updateButtonState(); // 버튼 상태 업데이트(02-07 추가!)


    // 배열 인덱스를 이용해 주사위 정보 출력
    const index = diceArray.indexOf(dice);
    console.log(
        `${index + 1}번째 주사위: ${result}면이 클릭됐습니다(${dice.body.userData.clickState})`
    );
