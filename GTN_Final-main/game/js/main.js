...
function createDiceMesh() {             
    const boxMaterialOuter = new THREE.MeshStandardMaterial({       // 주사위의 외형(boxMaterialOuter객체로 설정) 지정
        ...
        color: 0xeeeeee,                                            // 컬러는 밝은 회색
    })
    const boxMaterialInner = new THREE.MeshStandardMaterial({       // 주사위의 내부(boxMaterialInner객체로 설정) 지정(아래는 MeshStandardMateria의 속성)
        color: 0x000000,                                            // 컬러는 검은색
        roughness: 0,                                               // 재질 표면의 거칠기를 정의(작을수록 표면이 반짝이고 매끄러우며 반사광이 강해진다)
        metalness: 1,                                               // 재질 표면의 금속성을 정의(클수록 반사광이 강하며 반사되는 색상이 표면의 색상보다 강해진다)
...
function showRollResults(score) {               //showRollResults(score)는 주사위의 결과를 UI에 표시하는 역활을 한다.             
    if (scoreResult.innerHTML === '') {         // scoreResult는 HTML요소로, 주사위 결과를 표시하는 UI의 ID이다
                                                //   => HTML파일에 <span id="score-result"></span>로 저장되어있다.

                                                // innerHTML는 현재 scoreResult요소의 내용을 가져오거나 수정한다.
                                                // 즉 합치면 scoreResult.innerHTML === ''는 현재 결과 표시영역이 비어있는지 확인하는 코드이다.

        scoreResult.innerHTML += score;         // 결과 영역이 비어 있으면, 첫 번째 주사위의 결과를 추가
    } else {
        scoreResult.innerHTML += ('+' + score); // 만약 결과 영역이 비어있지 않으면 주사위의 결과값을 합산
    }
}
...
function throwDice() {
    scoreResult.innerHTML = '';                                                     // 화면의 점수를 초기화
